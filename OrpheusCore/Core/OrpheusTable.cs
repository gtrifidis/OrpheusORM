...
namespace OrpheusCore
{
    ...
    internal enum ModifiedRecordAction
    {
        mraInsert,
        mraUpdate,
        mraDelete
    ...
    internal delegate void ModelProperty(PropertyInfo property);
    #endregion
    ...
    public class OrpheusTable<T> : IOrpheusTable<T>
    {
        ...
        private ConcurrentDictionary<T, ConcurrentBag<KeyValuePair<string, object>>> insertCommandValues = new ConcurrentDictionary<T, ConcurrentBag<KeyValuePair<string, object>>>();
        ...
        /// <summary>
        /// Gets model's properties.
        /// </summary>
        /// <returns></returns>
        private PropertyInfo[] getModelProperties()
        {
            if (modelProperties == null)
                modelProperties = typeof(T).GetProperties();
        ...
        private List<ModelFieldInfo> getModelFields()
        {
            if (this.fields == null)
            {
                ...
                PropertyInfo[] properties = this.getModelProperties();
                for (var i = 0; i <= properties.Count() - 1; i++)
                {
                    ...
                    {
                        List<Attribute> attributes = (List<Attribute>)property.GetCustomAttributes(typeof(MaxLengthAttribute));
                        MaxLengthAttribute lenghAttribute = (MaxLengthAttribute)attributes.FirstOrDefault();
        ...
        private void processRecordForMasterDetail(T record, List<object> masterKeyValues)
        {
            if (this.masterTable != null && this.masterTableKeyFields != null)
            {
                ...
                {
                    var property = properties.FirstOrDefault(pr => pr.Name == this.masterTableKeyFields[i].Name);
        ...
        private void updateKeyValuesFromDB(T record)
        {
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    {
                        PropertyInfo propInfo = properties.FirstOrDefault(p => p.Name == dataReader.GetName(i));
                        if (propInfo != null)
                        {
        ...
        private void iterateModelProperties(ModelProperty callback)
        {
        ...
        /// <summary>
        /// Iterates the model to find all the primary keys.
        /// </summary>
        private void inferPrimaryKeys()
        {
            this.iterateModelProperties((PropertyInfo prop) => {
                foreach(var attr in prop.GetCustomAttributes(true))
                {
                    ...
                    {
                        if(this.KeyFields.Where(kf=>kf.Name.ToLower() == prop.Name.ToLower()).Count() == 0)
                        {
                            var newKeyField = OrpheusIocContainer.Resolve<IOrpheusTableKeyField>();
                            var primaryKey = (PrimaryKey)attr;

                            newKeyField.IsDBGenerated = primaryKey.IsDBGenerated;
                            newKeyField.IsAutoGenerated = primaryKey.IsAutoGenerated;
                            newKeyField.Name = prop.Name;
        ...
        /// <summary>
        /// Iterates the model to find all foreign keys
        /// </summary>
        private void inferForeignKeys()
        {
            this.iterateModelProperties((PropertyInfo prop) => {
                foreach (var attr in prop.GetCustomAttributes(true))
                {
                    ...
                    {
                        if (this.MasterTableKeyFields.Where(kf => kf.Name.ToLower() == prop.Name.ToLower()).Count() == 0)
                        {
                            var foreignKey = (ForeignKey)attr;
                            //master keys are not all foreign keys, but just the ones that belong to the master table.
                            if (this.MasterTable != null && this.MasterTable.Name.ToLower() == foreignKey.ReferenceTable.ToLower())
                            {
                                var newKeyField = OrpheusIocContainer.Resolve<IOrpheusTableKeyField>();
        ...
        private void processModifiedRecord(T modifiedRecord, ModifiedRecordAction action)
        {
            ...
            var newKeyListValuePair = new ConcurrentBag<KeyValuePair<string, Object>>();
            ...
            if (action == ModifiedRecordAction.mraInsert || action == ModifiedRecordAction.mraUpdate)
            {
                ...
                foreach (PropertyInfo property in properties)
                {
                    ...
                    if (canModifyRecord)
                    {
                        var paramValue = property.GetValue(modifiedRecord, null);
                        paramValue = paramValue == null ? DBNull.Value : paramValue;
                        if (keyField != null)
                        {
                            switch (action)
                            {
                                case ModifiedRecordAction.mraInsert:
                                    {
                                        ...
                                        if (keyField.IsAutoGenerated && property.PropertyType == typeof(System.Guid) && ((Guid)paramValue == Guid.Empty || paramValue == null || paramValue == DBNull.Value))
                                        {
                                ...
                                case ModifiedRecordAction.mraUpdate:
                                    {
                                        ...
                                        {
                                            newKeyListValuePair.Add(new KeyValuePair<string, object>(this.getKeyFieldParameterName(property.Name), existingKeyValue));
                                        }
                                        else
                                        {
                                            newKeyListValuePair.Add(new KeyValuePair<string, object>(this.getKeyFieldParameterName(property.Name), paramValue));
                                        }
                                        break;
                                    }
                            }
            ...
            if(action == ModifiedRecordAction.mraDelete)
            {
                ...
                {
                    var keyProperty = properties.Where(pf => pf.Name == keyField.Name).First();
                    if(keyProperty != null)
                    {
            ...
            switch (action)
            {
                case ModifiedRecordAction.mraInsert:
                    {
                        this.insertCommandValues.AddOrUpdate(modifiedRecord, newKeyListValuePair, (key, oldvalue) => newKeyListValuePair);
                        break;
                    }
                case ModifiedRecordAction.mraUpdate:
                    {
                        this.updateCommandValues.AddOrUpdate(modifiedRecord, newKeyListValuePair, (key, oldvalue) => newKeyListValuePair);
                        break;
                    }
                case ModifiedRecordAction.mraDelete:
                    {
                        this.deleteCommandValues.AddOrUpdate(modifiedRecord, newKeyListValuePair, (key, oldvalue) => newKeyListValuePair);
                        break;
                    }
            }
                ...
                while (dataReader.Read())
                {
                    ...
                    for (int i = 0; i <= fieldCount - 1; i++)
                    {
                        ...
                        if (propInfo != null)
                        {
                            ...
                            {
                                if (propInfo.PropertyType == typeof(System.Guid) && value.GetType() == typeof(System.String))
                                {
                                    value = Guid.Parse((string)value);
                                }

                            }
                            propInfo.SetValue(newInstance, value);
                        }
                    }
